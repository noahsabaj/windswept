# Clockwork to Windswept Migration

If you are here, you probably want to be converting your code from another framework to Windswept. Doing so should not be a difficult task. Most of the previous functions are probably within Windswept in one form or another! This means all you need to do is match *x* function found in the old framework to *y* function in Windswept. Some headings will contain a link - this will bring you to the documentation for Windswept's equivalent library or class.

This tutorial assumes basic to intermediate knowledge and experience with Garry's Mod Lua.

**Before you start!** You will notice that Windswept uses client for the variable that represents a player. Clockwork uses player for the variable instead, but this will conflict with the player library. So if you see `_player` being used in Clockwork, it means the Garry's Mod player library. This is just a preference and does not affect anything besides appear. So keep in mind throughout the tutorial, you may see player being used for Clockwork code and client being used for Windswept code. They represent the same thing, just with a different name.

If you are converting Clockwork code to Windswept, keep in mind that `_player` is not defined so you will need to either define `_player` yourself or switch it to player instead and change the variable name to client for player objects.

# Basics of Conversion

## Folders
Clockwork code and file structure is not too different from Windswept. In the schema, the plugins folder and schema folder stay in the same place. There are some minor differences in naming however:
...
## Deriving from Windswept
This is pretty important. If you want to use Windswept as the base, you need to set it as the base. So, go to your Clockwork schema's `gamemode` folder. Inside should be two files: `init.lua `and `cl_init.lua`. Open both, and you should see something along the lines of `DeriveGamemode("Clockwork")`. Change this to `DeriveGamemode("windswept")`.

# The Schema

## Introduction
Inside of the `schema` folder of the actual schema, you should see a file named `sh_schema.lua`. This is the main schema file in both Clockwork and Windswept. Most of your changes may actually be within this file.

## Including Files
Both frameworks come with a utility function to include a file without worrying about sending them to the client and stuff. In Clockwork, this function is `Clockwork.kernel:IncludePrefixed("sh_myfile.lua")`. Change this to `ws.util.Include("sh_myfile.lua") `and save.
...
## Structure
All plugins in Clockwork and Windswept go into the `plugins` folder. However, there are many differences with the CW plugin structure. First of all, there are two things you see when you open a plugin folder: `plugin` again and `plugin.ini`.

Windswept only has one file needed: `sh_plugin.lua` which acts like `sh_schema.lua` but for plugins.
...
-- after
ws.anim.SetModelClass("models/mymodel.mdl", "metrocop")
```

## Attributes (`ws.attributes`)
Attributes allow the player to boost certain abilities over time. Both frameworks require one to register attributes, but they are done differently. In Clockwork, the `ATTRIBUTE` table needs to be defined and registered manually. In Windswept, the `ATTRIBUTE` table is automatically defined and registered for you. All you need to do is have `ATTRIBUTE.value = "value"`. The basic parts of the attribute needed is `ATTRIBUTE.name` and `ATTRIBUTE.description`.

One extra feature for attributes in Windswept is `ATTRIBUTE:OnSetup(client, value)` which is a function that gets called on spawn to apply any effects. For example, the stamina attribute changes the player's run speed by adding the amount of stamina points the player has.
...
## Classes (`ws.class`)
Classes are a part of the factions. They basically are a more specific form of a faction. Factions in Windswept and Clockwork work similarly. For instance, all classes are placed in the `classes` folder under the schema folder and use `CLASS` as the main variable inside the file.
...
- `COMMAND.arguments` does not need to be defined if no arguments are needed but is defined as a table of argument types when needed `arguments = {ws.type.character, ws.type.number}`. See `ws.command.CommandArgumentsStructure` for details.
...
In Windswept, the method of adding configurations that can be changed by server owners is heavily simplified. [See an example here](https://github.com/NebulousCloud/helix/blob/master/gamemode/config/sh_config.lua).
...
- The 3rd argument for `Clockwork.config:AddToSystem(name, key, description, min, max)` is also the 3rd argument for `ws.config.Add`
- The 4th argument for `ws.config.Add` is an optional function that is called when the configuration is changed.
- The 5th argument for `ws.config.Add` is a table. You can specify the category for the configuration to group it with other configurations. There is also a data table inside which can be used to determine the minimum value and maximum value for numbers. Check out [an example here](https://github.com/NebulousCloud/helix/blob/master/gamemode/config/sh_config.lua). See also `ws.config`.
...
-- after
ws.currency.Set("", "token", "tokens")
ws.currency.Set("$", "dollar", "dollars")
```
...
## Factions (`ws.faction`)
Factions, like classes, are pretty similar too. They share pretty much the same differences as classes in Clockwork and Windswept do.
...
## Flags (`ws.flag`)
Flags are functionally equivalent in Windswept. To add a new flag:
...
To access a player's inventory, you need to use `client:GetCharacter():GetInventory()` which returns the main inventory object for the player's character. You can also access all loaded inventories with `ws.item.inventories` but that is not important right now.
...
### Item Sizes
Windswept's inventory uses a grid and utilizes width and height instead of weight as a means of inventory capacity. This means you will have to change your item's weight (`ITEM.weight`) to something that might be analagous to the item's size using `ITEM.width` and `ITEM.height`. The item's size must be at least one by one grid cell. It's up to you to balance the sizes of items in your use case - taking into account how many items a character might have at once, the default inventory size set in the config, etc.
...
And this would create a new instance of an item. Windswept's instancing system is slightly different. Instead of having the function return the instance like it does in Clockwork, Windswept relies on a callback to pass the instance. The reason for this is the item must be inserted into the database to get a unique number to represent that item. This is not done instantly, otherwise servers would freeze when new items are made. Clockwork uses the time and adds a number to get the numeric ID for an item, which allows the item to be returned which "solves" the issue, but I digress.

The Windswept equivalent would be:

```
ws.item.Instance(0, "item", data, x, y, function(item) end)
```

Let's break down the differences:

- For Windswept's item instance, the 1st argument (`0`) is the inventory that the item belongs to. You can specify 0 so it does not belong to any inventory.
...
Keep in mind that Windswept will simplify the item system for you when it can. Normally, you would not need to instance an item yourself unless you were doing something advanced.
...
The equivalent in Windswept would be:

```
-- spawning an item in the map
ws.item.Spawn("item", Vector(1, 2, 3))

-- giving a player an item
client:GetCharacter():GetInventory():Add("test")
```

So in these two examples, the whole deal of instancing items is done for you in Windswept!
...
# Conclusion
Overall, most of the conversion from Clockwork to Windswept is simply renaming a certain function and/or switching the order of arguments around. Both are frameworks so they function similarly.
