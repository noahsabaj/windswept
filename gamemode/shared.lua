...
-- @table ws.type
-- @realm shared
-- @field string A regular string. In the case of `ws.command.Add`, this represents one word.
-- @field text A regular string. In the case of `ws.command.Add`, this represents all words concatenated into a string.
-- @field number Any number.
-- @field player Any player that matches the given query string in `ws.util.FindPlayer`.
-- @field steamid A string that matches the Steam ID format of `STEAM_X:X:XXXXXXXX`.
-- @field character Any player's character that matches the given query string in `ws.util.FindPlayer`.
...
-- only supported in `ws.option.Add`.
-- @see ws.command.Add
-- @see ws.option.Add
...
-- print(ws.type[ws.type.string])
-- > "string"
ws.type = ws.type or {}

-- Define gamemode information.
GM.Name = "Windswept Framework"
GM.Author = "Noah Sabaj"
GM.Website = "https://github.com/noahsabaj"
GM.Version = "0.1.0"

do
	-- luacheck: globals player_manager
	player_manager.wsTranslateModel = player_manager.wsTranslateModel or player_manager.TranslateToPlayerModelName

	function player_manager.TranslateToPlayerModelName(model)
		model = model:lower():gsub("\\", "/")
		local result = player_manager.wsTranslateModel(model)

		if (result == "kleiner" and !model:find("kleiner")) then
			local model2 = model:gsub("models/", "models/player/")
			result = player_manager.wsTranslateModel(model2)
...
ws.util.Include("core/cl_skin.lua")
ws.util.IncludeDir("core/libs/thirdparty")
ws.util.Include("core/sh_config.lua")
ws.util.IncludeDir("core/libs")
