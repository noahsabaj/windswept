...
function PLUGIN:SetupActs()
	-- sit
	ws.act.Register("Sit", {"citizen_male", "citizen_female"}, {
	...
	ws.act.Register("SitWall", {"citizen_male", "citizen_female"}, {
		sequence = {
	...
	ws.act.Register("Sit", "vortigaunt", {
		sequence = "chess_wait",
		untimed = true,
		idle = true
	})
	...
	-- stand
	ws.act.Register("Stand", "citizen_male", {
		sequence = {"lineidle01", "lineidle02", "lineidle03", "lineidle04"},
		untimed = true,
		idle = true
	})

	ws.act.Register("Stand", "citizen_female", {
		sequence = {"lineidle01", "lineidle02", "lineidle03"},
		untimed = true,
		idle = true
	})

	ws.act.Register("Stand", "metrocop", {
		sequence = "plazathreat2"
	})

	-- cheer
	ws.act.Register("Cheer", "citizen_male", {
		sequence = {{"cheer1", duration = 1.6}, "cheer2", "wave_smg1"}
	})

	ws.act.Register("Cheer", "citizen_female", {
		sequence = {"cheer1", "wave_smg1"}
	})

	-- lean
	ws.act.Register("Lean", {"citizen_male", "citizen_female"}, {
	...
	-- arrest
	ws.act.Register("ArrestWall", "citizen_male", {
		sequence = {
			{"apcarrestidle",
			check = FacingWall,
	...
	ws.act.Register("Arrest", "citizen_male", {
		sequence = "arrestidle",
		untimed = true
	})

	-- threat
	ws.act.Register("Threat", "metrocop", {
		sequence = "plazathreat1",
	})

	-- deny
	ws.act.Register("Deny", "metrocop", {
		sequence = "harassfront2",
	})

	-- motion
	ws.act.Register("Motion", "metrocop", {
		sequence = {"motionleft", "motionright", "luggagewarn"}
	})

	-- wave
	ws.act.Register("Wave", {"citizen_male", "citizen_female"}, {
		sequence = {{"wave", duration = 2.75}, {"wave_close", duration = 1.75}}
	})

	-- pant
	ws.act.Register("Pant", {"citizen_male", "citizen_female"}, {
	...
	-- window
	ws.act.Register("Window", "citizen_male", {
		sequence = "d1_t03_tenements_look_out_window_idle",
		untimed = true
	})

	ws.act.Register("Window", "citizen_female", {
