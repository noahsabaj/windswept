...
if (SERVER) then
	function PLUGIN:GetPlayerPunchDamage(client, damage, context)
		if (client:GetCharacter()) then
			...
			context.damage = context.damage + (client:GetCharacter():GetAttribute("str", 0) * ws.config.Get("strengthMultiplier"))
		end
	end
...
ws.config.Add("strengthMultiplier", 0.3, "The strength multiplier scale", nil, {
